import React from 'react'
import StoryTag from './StoryTag'
import { AiFillStar, AiFillLike } from "react-icons/ai"
import  { BsFillEyeFill } from "react-icons/bs"

function StoryInfo() {
	return (
		<>
		<div className="container flex justify-between mb-16">
			<div className="flex flex-col">
				<h1 className="text-3xl font-semibold mb-4">The Drift</h1>
				<div className="flex">
					<StoryTag title="in progress" color="primary-red" marginRight="2"/>
					<StoryTag title="adventure" color="primary-blue" marginRight="2"/>
					<StoryTag title="action" color="primary-blue" marginRight="2"/>
				</div>
			</div>

			<div className="flex flex-col">
				<div className="flex items-center mb-4">
					<img className="w-10 h-10 rounded-full bg-red-100 object-cover mr-4" />
					<h2 className="text-xl font-medium">Ali Mejbri</h2>
				</div>
				<button className="text-center font-semibold w-24 py-1 btn btn-blue ml-auto hover:bg-primary-blue-darker">Follow</button>
			</div>
			
		</div>

		<img className="object-cover w-full h-xl mb-4" src="data:image/jpeg;base64," />

		<div className="container flex justify-between mb-12">
			<div className="flex items-center">
				<div className="p-1 text-sm font-medium text-center bg-gray-300 rounded-md mr-2">4.8</div>
				<AiFillStar className="h-6 fill-yellow"/>
				<AiFillStar className="h-6 fill-yellow"/>
				<AiFillStar className="h-6 fill-yellow"/>
				<AiFillStar className="h-6 fill-yellow"/>
				<AiFillStar className="h-6 fill-yellow"/>
			</div>

			<div className="flex align-center">
				<div className="flex items-center mr-6">
					<BsFillEyeFill className="h-6 mr-2 fill-red" />
					<h2 className="text-md">12K Views</h2>
				</div>
				<div className="flex items-center">
					<AiFillLike className="h-6 mr-2 fill-blue"/>
					<h2 className="text-md">1.2K Likes</h2>
				</div>
			</div>
		</div>
		</>
	)
}

export default StoryInfo
